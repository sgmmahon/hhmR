...
Below is an example of the `hhm` function's application. For a more in depth description of it's usage, see `url of vignette`.
...
Below is an of how the `tshhm` function can be used. For a more details, see `url of vignette`.
```
# Import packages
library(dplyr)
