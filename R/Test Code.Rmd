...
#' round the legend scale to (defaults to 3).
#' @param bins If breaking legend into discrete ranges and not using custom 
#' breaks, how many colours to break the legend colour scheme into (defaults to 
#' 15).
#'
#' @return A ggplot object containing the final heatmap.
#' 
...
hhm = function(df,ylower,yupper,xlower,xupper,values,rm_diag=F,bins=15,lgttl=NULL,cbrks=NULL,cclrs=NULL,norm_brks=F,lgdps=3) {
  ...
  xgrps = df[[xupper]] %>% unique()
  ygrps = df[[yupper]] %>% unique()
  ...
  if (norm_brks) {
    ...
    if ((df[[values]] < 0) %>% sum() %>% {. > 0}) {stop("norm_brks can only be used if all values used to populate the heatmap are positive.")}
  ...
  for (ygrp in 1:length(ygrps)) {
    ...
    for (xgrp in 1:length(xgrps)) {
      ...
      # Reverse order of categories on y-axis to ensure goes alphabetically from top to bottom
      sdf[[ylower]] = factor(sdf[[ylower]], levels = sdf[[ylower]] %>% unique() %>% rev() )
      
      # Define main plot
      p = ggplot(sdf, aes_string(xlower, ylower)) +
        geom_tile(aes_string(fill = values), show.legend = T) +
      ...
      if (!is.null(cbrks) && !is.null(cclrs)) {
        p = p + scale_fill_manual(name = lgttl, values = cg("white","#08306B",bns), drop = F)
